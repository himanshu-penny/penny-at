...
> **penny-at override:** where this document says
> `import { test, expect } from "./fixtures"` or `@playwright/test`, use
> `import { test, expect } from "@fixtures";` instead — the framework merges
> base, api, and web fixtures via `mergeTests()` in `src/fixtures/index.ts`.
> Also route web interactions through page objects that extend `BasePage`, not
> raw `page.*` calls in specs. See `/penny-create-web-script` for the
> penny-at test shape.
