...
Use these workspace skills for API-focused script creation and review:

- `penny-api-sdet` — source of truth for Penny API framework patterns, auth, retry,
  redaction, schema validation, fixtures, and constants.
- `penny-create-api-script` — guided workflow for creating new API Playwright specs.
- `penny-review-api-script` — review checklist for API specs and API framework changes.
- `penny-create-utility-script` — workflow for Node/Bash helper scripts,
  setup/cleanup scripts, schema generators, and report utilities.

Prefer these local API-first skills over UI-heavy patterns from other Penny repos.
